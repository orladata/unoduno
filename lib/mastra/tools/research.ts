...
export const searchWebForTrendsTool = createTool({
  ...
  execute: async ({ query }: { query: string }) => {
    try {
      console.log(`[searchWebForTrendsTool] Analisando tendências para: ${query}`);
      ...
      const mockTrends = [
        `As buscas por "${query}" revelam que o público brasileiro está preferindo conteúdos rápidos, diretos ao ponto e com ganchos visuais nos primeiros 3 segundos.`,
        `Termos em alta relacionados: "Como ganhar dinheiro com ${query}", "Erros comuns em ${query}".`,
